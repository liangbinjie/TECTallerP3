...
def formulario_insercionMedicacion():
    ...
    Label(t ,text = "Dia").grid(row=2, column = 0)
    Label(t ,text = "Mes").grid(row=3, column = 0)
    Label(t ,text = "Ano").grid(row=4, column = 0)
    Label(t ,text = "Lista de Medicamentos").grid(row=5, column = 0)
    Label(t ,text = "Costo unitario").grid(row=6, column = 0)
    Label(t ,text = "Cantidad").grid(row=7, column = 0)
    ...
    dia = Entry(t)
    dia.grid(row = 2,column = 1)
    mes = Entry(t)
    mes.grid(row = 3,column = 1)
    ano = Entry(t)
    ano.grid(row =4,column = 1)
    ListaDeMedicamentos = Entry(t)
    ListaDeMedicamentos.grid(row = 5,column = 1)
    CostoUnitario = Entry(t)
    CostoUnitario.grid(row = 6,column = 1)
    Cantidad = Entry(t)
    Cantidad.grid(row = 7,column = 1)

    from medicacion import insercionmedicacion

    Button(t , text="Guardar", command = lambda: insercionmedicacion(Codigo_de_Animal.get(),codigoDeMedicacion.get(),dia.get(),mes.get(),ano.get(),ListaDeMedicamentos.get(),CostoUnitario.get(),Cantidad.get())).grid(row=8,column=1)